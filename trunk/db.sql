...
CREATE VIEW `view_t_open` AS select `t`.`id` AS `id`,`t`.`subject` AS `subject`,`t`.`added` AS `added`,`t`.`priority` AS `priority`,concat(`u`.`firstname`,_utf8' ',`u`.`lastname`) AS `addername`,`u`.`phone` AS `adderphone`,`u`.`email` AS `adderemail`,`c`.`name` AS `companyname` from ((`TICKETS` `t` join `USERS` `u`) join `COMPANIES` `c`) where ((`t`.`companyid` = `c`.`id`) and (`t`.`userid` = `u`.`id`) and (`t`.`ticketstatus` = 1)) group by `t`.`priority`,`t`.`customorder`,`t`.`id` order by `t`.`priority`,`t`.`customorder`,`t`.`id`;
CREATE VIEW `view_t_replies` AS select `r`.`id` AS `id`,`r`.`ticketid` AS `ticketid`,`r`.`descr` AS `descr`,`r`.`userid` AS `userid`,`r`.`added` AS `added`,`r`.`replytype` AS `replytype`,concat(`u`.`firstname`,_utf8' ',`u`.`lastname`) AS `writer`,`u`.`email` AS `writeremail`,`u`.`phone` AS `writerphone` from ((`TICKET_REPLY` `r` join `TICKETS` `t`) join `USERS` `u`) where ((`r`.`ticketid` = `t`.`id`) and (`r`.`userid` = `u`.`id`)) group by `r`.`added`,`r`.`id` order by `r`.`added`,`r`.`id`;
